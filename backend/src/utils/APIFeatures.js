...
// The listings page has filters, a search box and pages. Doing all that inside the controller would make it 100 lines long.
// So we keep it here, and the controller stays clean:
// new APIFeatures(Property.find(), req.query).filter().search().paginate()
...
class APIFeatures {
    ...
    // queryString = what the user asked for (req.query), the part of the address after the ? mark
    ...
    filter() {
        ...
        // PRICE. $gte = greater than or equal to. $lte = less or equal.
        ...
        // TYPE. It arrives as one string 'House, Flat', so we cut it at the commas and trim the spaces. $in = match any of them.
        if(queryObj.propertyType) {
            let propertyTypeArray = queryObj.propertyType.split(",").map((value) => value.trim());
        ...
        // AMENITIES. May arrive as one value or as a list, so we force it into a list either way.
        if(queryObj.amenities) {
            const amenitiesArray = Array.isArray(queryObj.amenities)? queryObj.amenities : [queryObj.amenities];

            // the dot goes INSIDE the amenities object.
            filterQuery["amenities.name"] = { $all: amenitiesArray };
        }

        // add these rules to the search. Nothing runs yet - mongoose only collects the rules until we await it.
        this.query = this.query.find(filterQuery);
        // return this = give the object back, so the next method can be joined with a dot: .filter().search().paginate()

        return this;

    }

    // SEARCH - the box on topL city, guests, dates
    search() {
        let searchQuery = {};
        let queryObj = {...this.queryString};

        // CITY. $or = any of these may match. We compare in small letters with no spaces, because the model saves the city that way too ('New Delhi' -> 'newdelhi'). 
        // No city typed -> empty {}
        ...
        // DATES - the hardest part. We must hide houses that are already booked on those days
        // $elemMatch looks inside the currentBookings list,
        // $not turns it around: show me the ones where NO booking clashes with the dates the guest asked for.
                        ...
                        $not: {
                            $elemMatch: {
                                $or: [
                                    ...
                                    // conflict logic: fromDate < dateOut && toDate > dateIn
    ...
    paginate() {
        ...
        // This is the main pagination formula.

        // It tells MongoDB how many records to ignore before starting to return results.
